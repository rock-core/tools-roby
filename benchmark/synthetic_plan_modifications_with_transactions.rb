...
def randomly_modify_plan(plan, num_tasks, num_relation_changes, num_mission_changes, commit: true)
    plan.in_transaction do |trsc|
        ...
        if plan.size < num_tasks
            (num_tasks - plan.size).times do
...
synthetic_plan_modifications_count = 10
Benchmark.bm(70) do |x|
    num_tasks            = 100
    num_relation_changes = 500
    num_mission_changes  = 10

    x.report "modifies an executable plan using a transaction (#{synthetic_plan_modifications_count} times)" do
        plan = Roby::Plan.new
        Roby::ExecutionEngine.new(plan)

        synthetic_plan_modifications_count.times do |i|
            randomly_modify_plan(plan, num_tasks, num_relation_changes, num_mission_changes, commit: false)
        end
    end

    x.report "modifies and commits an executable plan using a transaction (#{synthetic_plan_modifications_count} times)" do
        plan = Roby::Plan.new
        Roby::ExecutionEngine.new(plan)
