...
module Roby
    ...
    class Plan < DistributedObject
        ...
        LocalQueryResult = Struct.new :plan, :initial_set, :result_set do
            def include?(obj)
        ...
        def query_result_set(matcher) # :nodoc:
            filtered = matcher.filter(
                tasks, task_index, initial_is_complete: true
            )

            if matcher.indexed_query?
                PlanQueryResult.new(self, tasks, filtered)
            else
                result = Set.new
                filtered.each do |task|
                    result << task if matcher === task
                end
                PlanQueryResult.new(self, tasks, result)
            end
        end
        ...
        def query_roots(result_set, relation) # :nodoc:
            ...
            new_results = result_set.each_in_plan(self).find_all do |task|
                root_in_query?(result_set, task, graph)
            end
            PlanQueryResult.new(self, tasks, new_results.to_set)
        end
