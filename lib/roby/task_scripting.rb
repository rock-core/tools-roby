module Roby
    ...
    module TaskCoordination
        ...
        module Models
            class PollUntil
                ...
                def new(script)
                    TaskCoordination::PollUntil.new(script.root_task, script.instance_for(event), block)
                end
            end
        end
        ...
        class PollUntil
            ...
            def execute(script)
                ...
                event.resolve.when_unreachable(true) do |reason, generator|
                    raise Actions::Script::DeadInstruction.new(script.root_task), "the 'until' condition of #{self} will never be reached: #{reason}"
                end
                false
            end
        end

        class Script < Actions::ExecutionContext
            extend Actions::Models::Script
            include Actions::ScriptBase
            ...
            def resolve_instructions
                ...
                instructions.each do |ins|
                    case ins
                    when Actions::Models::Script::Start
            ...
            def resolve_task(task)
                if task.kind_of?(Roby::Task)
                    model_task = Roby::Actions::Models::ExecutionContext::Task.new(task.model)
                    script_task = instance_for(model_task)
                    script_task.bind(task)
                else
            ...
            def poll_until(event, &block)
                event, model_event = resolve_event(event)
                model.instructions << Models::PollUntil.new(model_event, block)
                event
            end
    ...
    class Task
        ...
        def self.create_script(*task, &block)
            script_model = TaskCoordination::Script.new_submodel(self)
