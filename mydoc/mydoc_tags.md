...
You can change the button color by changing the class on the button from `btn-info` to one of the other button classes bootstrap provides. See {{site.data.mydoc_urls.mydoc_labels.link}} for more options on button class names.
...
See {{site.data.mydoc_urls.mydoc_webstorm_text_editor.link}} for tips on creating file templates in WebStorm.
