...
class TC_Propagation < Test::Unit::TestCase
	...
	assert_equal({ e1 => [false, [], [1]], e2 => [true, [], [2, 3]] }, set)
    end
	...
	assert_equal(false, set[e1][0])
	assert_equal([s1, s2].to_set, set[e1][1].map { |e| e.generator }.to_set)
	assert_equal([1, 2].to_set, set[e1][2].to_set)

	assert(set.has_key?(e2))
	assert_equal(true, set[e2][0])
	assert_equal([s2], set[e2][1].map { |e| e.generator })
	assert_equal([2], set[e2][2])
    end
