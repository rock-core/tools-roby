            ...
            describe ".parse_arguments_respectively" do
                it "uses the first argument's keys as default names for the parsed values" do
                    assert_equal Hash[x: 10, y: 20],
                        CucumberHelpers.parse_arguments_respectively([:x, :y], "10m and 20m", strict: false)
                end
                it "applies the same value to all keys if a single numerical value is provided" do
                    assert_equal Hash[x: 10, y: 10],
                        CucumberHelpers.parse_arguments_respectively([:x, :y], "10m", strict: false)
                end
                it "does not allow to mix name and order" do
                    assert_raises(CucumberHelpers::MixingOrderAndNames) do
                        CucumberHelpers.parse_arguments_respectively([:x, :y], "y=10m and 20m", strict: false)
                    end
                    assert_raises(CucumberHelpers::MixingOrderAndNames) do
                        CucumberHelpers.parse_arguments_respectively([:x, :y], "10m and x=20m", strict: false)
                    end
                end
                it "allows to pass the arguments by name" do
                    assert_equal Hash[y: 10, x: 20],
                        CucumberHelpers.parse_arguments_respectively([:x, :y], "y=10m and x=20m", strict: false)
                end
                it "raises if names not present in the reference hash are given" do
                    exception = assert_raises(CucumberHelpers::UnexpectedArgument) do
                        CucumberHelpers.parse_arguments_respectively([:x, :y], "z=10m", strict: false)
                    end
                    assert_equal "got 'z' but was expecting one of x, y",
                        exception.message
                    exception = assert_raises(CucumberHelpers::UnexpectedArgument) do
                        CucumberHelpers.parse_arguments_respectively([:x, :y], "x=10m, y=10m and z=10m", strict: false)
                    end
                    assert_equal "got 'z' but was expecting one of x, y",
                        exception.message
                end
                ...
                describe "unit validation" do
                    describe "fully implicit mode" do
                        it "raises UnexpectedArgument if strict is set and the argument does not have a quantity" do
                            assert_raises(CucumberHelpers::UnexpectedArgument) do
                                CucumberHelpers.parse_arguments_respectively([:x, :y], "20m", Hash[x: :length], strict: true)
                            end
                        end
                        it "validates that the unit and the quantity match" do
                            flexmock(CucumberHelpers).should_receive(:validate_unit).
                            ...
                            CucumberHelpers.parse_arguments_respectively([:x, :y], "20m", Hash[x: :length, y: :angle], strict: false)
                        end
                        it "raises InvalidUnit if the value does not have a unit" do
                            e = assert_raises(CucumberHelpers::InvalidUnit) do
                    ...
                    describe "name-based mode" do
                        it "raises UnexpectedArgument if strict is set and the argument does not have a quantity" do
                            assert_raises(CucumberHelpers::UnexpectedArgument) do
                                CucumberHelpers.parse_arguments_respectively([:x, :y], "x=20m and y=5m", Hash[x: :length], strict: true)
                            end
                        end
                        it "validates that the unit and the quantity match" do
                            flexmock(CucumberHelpers).should_receive(:validate_unit).
                            ...
                            CucumberHelpers.parse_arguments_respectively([:x, :y], "y=20m and x=5m", Hash[x: :length, y: :angle], strict: false)
                        end
                        it "raises InvalidUnit if the value does not have a unit" do
                            e = assert_raises(CucumberHelpers::InvalidUnit) do
                                CucumberHelpers.parse_arguments_respectively([:x, :y], "x=20m and y=5", Hash[x: :length, y: :length], strict: false)
                            end
                    ...
                    describe "order-based mode" do
                        it "raises UnexpectedArgument if strict is set and the argument does not have a quantity" do
                            assert_raises(CucumberHelpers::UnexpectedArgument) do
                                CucumberHelpers.parse_arguments_respectively([:x, :y], "20m and 5m", Hash[x: :length], strict: true)
                            end
                        end
                        it "raises InvalidUnit if the unit does not match the expected quantity" do
                            ...
                            flexmock(CucumberHelpers).should_receive(:validate_unit).
                                with(:x, 20.0, 'm', :length).once
                            CucumberHelpers.parse_arguments_respectively([:x, :y], "20m and 5m", Hash[x: :length, y: :angle], strict: false)
                        end
                        it "raises InvalidUnit if the value does not have a unit" do
                            e = assert_raises(CucumberHelpers::InvalidUnit) do
                                CucumberHelpers.parse_arguments_respectively([:x, :y], "20m and 5", Hash[x: :length, y: :length], strict: false)
                            end
                            assert_equal "expected y=5 to be a length",
                                e.message
                        end
