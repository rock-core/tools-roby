    ...
    module Interface
        describe REST do
            ...
            describe "the start timeout behavior" do
                ...
                it "raises if the thread is not functional after the alloted timeout" do
                    @server = REST::Server.new(@app, port: 0)
                    flexmock(@server).should_receive(:create_thin_thread).and_return { Thread.new {} }
            ...
            describe "over TCP" do
                describe "a given port of zero" do
                    ...
                    describe "#port" do
                        it "returns the port right away if the server is started and synchronized" do
                            @server.start
                            refute_equal 0, @server.port(timeout: 0)
                        end
                        it "waits for the port to be available if the server is started" do
                            @server.start(wait_timeout: 0)
                        ...
                        it "returns the actual port if the server has been started and then stopped" do
                            @server.start
                            @server.stop
                ...
                describe "a nonzero port" do
                    ...
                    describe "#port" do
                        ...
                        it "returns the port if the server is started but we didn't synchronize" do
                            @server.start(wait_timeout: 0)
                            assert_equal @port, @server.port
                        end

                        it "returns the port if the server is started and synchronized" do
                            @server.start
                            assert_equal @port, @server.port
                        end
